...
| int32_t EnforceCodeSignForAppWithOwnerId(const std::string &path, const EntryMap &entryPathMap, FileType type, const ByteBuffer &profileBuffer, uint32_t flag); | Enforces code signing for HAPs with the owner ID.|
...
| int InitXpmWithParam(const struct XpmInitParam *initParam); | Initializes XPM resources using grouped struct parameters (preferred).|
| int InitXpm(int enableJitFort, uint32_t idType, const char *ownerId, const char *apiTargetVersionStr, const char *appSignType); | Compatibility wrapper for legacy callers.|
